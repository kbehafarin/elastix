...
if (`ps aux | grep "updatecdr" | grep -v "grep"` =~ /updatecdr/) {
	...
	$dbh->prepare("delete from globals where variable='MIXMON_POST'")
		->execute();
	$dbh->prepare("insert into globals values (variable,value) values (?, ?)")
	...
	system("chmod a+x $basedir/elastix/bin/updatedcr && ln -s $basedir/elastix/bin/updatecdr /usr/bin/updatecdr");
	...
	system("echo \"create database qstats\" | mysql -u $user --password=$pass");
	system("grant all on qstats.* to '$user'\@'localhost' identified by '$pass'\" | mysql -u $user --password=$pass");
